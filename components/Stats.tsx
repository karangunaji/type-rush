...
export default function Stats({ wpm, accuracy, timeLeft, typedWords, totalWords }: Props) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 mb-6">
      ...
      <div className="rounded-3xl border border-surface-strong bg-surface p-5 text-center shadow-lg shadow-slate-950/20">
        ...
        <p className="mt-3 text-4xl font-semibold text-danger">{formattedTime}</p>
      </div>

      <div className="rounded-3xl border border-surface-strong bg-surface p-5 text-center shadow-lg shadow-slate-950/20">
        <p className="text-sm uppercase tracking-[0.2em] text-muted">Words</p>
        <p className="mt-3 text-2xl font-semibold text-warning">{typedWords} / {totalWords}</p>
      </div>
    </div>
  );
}